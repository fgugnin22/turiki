...
const TournamentList = () => {
  ...
  return (
    <Layout>
      {
        <section>
          <h1 className="text-5xl text-center mt-8 text-lightgray">Турниры</h1>
          {isSuccess && data?.length > 0 ? (
            <div className="mx-auto mt-12 text-lightgray">
              {!userDetails?.is_staff && (
                <div
                  className={`shadow mx-auto w-full relative mb-4 py-2 rounded-md flex text-center justify-between items-center gap-x-2`}
                >
              ...
              {data?.map((tourn, index) => (
                <div className="flex gap-x-2 text-black" key={index}>
                  <Link
                    ...
                    className={`shadow mx-auto w-full relative border mb-4 hover:bg-slate-200 transition h-24 bg-white dark:bg-gray-800 rounded-md flex text-center justify-between  items-center `}
                  >
                    <h2 className="text-sm md:text-lg w-[20%] min-w-[110px]">
                      {tourn.name}
                    </h2>
                    ...
                    <h3 className="text-sm  font-light md:text-md w-[25%] min-w-[100px]">
                      {(tourn.status === "REGISTRATION_CLOSED_BEFORE_REG" &&
                        "Регистрация ещё закрыта") ||
                        (tourn.status === "REGISTRATION_OPENED" &&
                          "Регистрация открыта") ||
                        (tourn.status === "REGISTRATION_CLOSED_AFTER_REG" &&
                          "Регистрация уже закрыта") ||
                        (tourn.status === "CHECK_IN" &&
                          "Подтверждение(check-in)") ||
                        (tourn.status === "CHECK_IN_CLOSED" &&
                          "Скоро начнётся") ||
                        (tourn.status === "ACTIVE" && "Активен") ||
