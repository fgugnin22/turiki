            ...
            <div className="mx-auto w-full lg:w-4/5 xl:w-[1100px] flex align-bottom flex-wrap">
                ...
                <p className=" text-white opacity-50 leading-6">
                  {(tournament.status === "REGISTRATION_CLOSED_BEFORE_REG" &&
                    ...
                    (tournament.status === "REGISTRATION_OPENED" &&
                      "Регистрация открыта") ||
                    (tournament.status === "REGISTRATION_CLOSED_AFTER_REG" &&
                      "Регистрация уже закрыта") ||
                    (tournament.status === "CHECK_IN" &&
                      "Подтверждение(check-in)") ||
                    (tournament.status === "CHECK_IN_CLOSED" &&
                      "Скоро начнётся") ||
                    (tournament.status === "ACTIVE" && "Активен") ||
                    (tournament.status === "PLAYED" && "Сыгран") ||
              ...
              <div className="w-1/2 bg-transparent flex flex-wrap justify-end">
                <p className="w-full text-right text-white">
                  <span className=" opacity-50">До конца регистрации: </span>
                  <span className=" text-lightblue">00:12:32</span>
                </p>
                {isTeamNotRegistered && (
              ...
              <div className="w-full font-roboto font-medium text-base flex justify-center gap-12 items-center">
                <NavLink
                  className={({ isActive }) =>
                    `${
                      isActive
